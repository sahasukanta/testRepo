...
def main():
    ...
    gitToken = os.environ["TEST_SECRET"]
    ...
    # # adding university column to df
    # uniCol = ["ualberta"]*1372
    # df["university"] = uniCol
    # df = df[["university", "journal", "issn", "access", "notes"]]

    oldDB, updatedMainDB = mergeMainDB(repo, "mainDB.csv", df)
    # # merging with mainDB.csv
    # oldDB = repo.get_contents("mainDB.csv")
    # oldDB_content = oldDB.decoded_content  # dtype=bytes
    # oldDB_content = pd.read_csv(BytesIO(oldDB_content))  # dtype=pd.df
    # oldDB_content = oldDB_content.astype("string")
    # oldDB_content = oldDB_content[["university", "journal", "issn", "access", "notes"]]
    # updatedDB = pd.concat([df, oldDB_content], ignore_index=True)

    updateMainDBGit(repo, oldDB, updatedMainDB, "mainDB.csv")
    # # updating mainDB on github
    # updatedDB = updatedDB.to_csv()  # dtype=str
    # repo.delete_file(oldDB.path, "commit message", oldDB.sha)
    # repo.create_file("mainDB.csv", "test Commit", updatedDB)  # uploading merged df as mainDB.csv
